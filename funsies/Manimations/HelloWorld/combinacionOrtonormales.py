...
class combinacionOrtonormales(Scene):
    def construct(self):
        # Ejes
        ax = Axes(
            x_range=[-2, 6, 1],
        ...
        # Vectores A, B y X
        A = np.array([0.7071, 0.7071, 0])
        B = np.array([-0.7071, 0.7071, 0])
        X = np.array([4, 3, 0])
        ...
        labelA = MathTex("a", color=BLUE).next_to(vecA.get_end())
        ...
        # Etiquetas
        label_lambdaA = MathTex(r"\lambda a", color=BLUE).next_to(dot_lambdaA, LEFT)
        label_muB = MathTex(r"\mu b", color=GREEN).next_to(dot_muB, UP)
        ...
        label_lambdaA = MathTex(r"\lambda a", color=BLUE_B).next_to(p_lambdaA, LEFT)
        label_muB = MathTex(r"\mu b", color=GREEN_B).next_to(p_X, UP)
