...
class MediasInequalities2(Scene):
    def construct(self):
        A = np.array([-2.5, 0, 0])
        B = np.array([-0.5, 0, 0 ])
        C = np.array([2.5, 0 , 0 ])
        E = np.array([-0.5, 2.44949 , 0 ])
        F = np.array([-0.5, 2.44949 , 0 ])
        O = np.array([0, 0, 0])
        ...
        labelE = MathTex("E").next_to(E).shift(0.5*UP)
        ...
        self.play(FadeIn(labelE))

        self.wait(1)
        l5= Line(O, E)
        self.play(FadeIn(l5), run_time = 2)
        self.wait(1)
        brace3 = Brace(l4, LEFT)
        label3 = brace3.get_text("h")
        self.play(GrowFromCenter(brace3), FadeIn(label3))
        self.wait(2)
        self.play(FadeOut(brace3), FadeOut(label3))
        self.wait(2)

        # Calculamos la perpendicular: usamos el vector dirección de la línea
        line_dir = E - O           # vector dirección de la línea original
        perp_dir = np.array([-line_dir[1], line_dir[0], 0])  # vector perpendicular

        # Definimos la perpendicular pasando por P
        length = 4  # longitud de la perpendicular
        perp_line = Line(B - perp_dir/2, B + perp_dir/2)

        # Dibujamos la perpendicular
        self.play(Create(perp_line))
        labelF = MathTex("F").next_to(perp_line).shift(0.5*UP)

        self.wait(1)
        self.play(FadeIn(labelF), run_time = 2)
        self.wait(1)
